...
def item_description_fetch(item_id):
    access_url = "https://store.steampowered.com/app/"+str(item_id)
    requests_url = requests.get(access_url)
    requests_url.encoding = requests_url.apparent_encoding
    try:
        bs_object=BeautifulSoup(requests_url.content, "html.parser")
        # print(requests_url.content)
        # description_area_object = bs_object.select(id="game_area_description")
        description_area_object = bs_object.find(id="game_area_description")
        # print(description_area_object)
        item_description_text = description_area_object.get_text()
    except:
        print('error発生')
        return ""
    # print(item_description_text)
    return item_description_text

def format_item_description(item_description_text):
    formated_text = item_description_text.replace("\n","")
    formated_text = formated_text.replace("\t","")
    formated_text = formated_text.replace("\"","\'")
    formated_text = formated_text.replace("About This Game","",1)

    # print(formated_text)
    return formated_text

def item_description_fetch_main(db_path):
    con = sqlite3.connect(db_path)
    cur = con.cursor()

    #descriptionが空のものにのみ適応
    cur.execute('select item_id from item where description is null order by item_id ')
    #↓すべてのitemに適応
    # cur.execute('select item_id from item order by item_id ')

    columns = cur.fetchall()
    for column in columns:
        item_id = str(column[0])
        print(item_id)
        item_description_text = item_description_fetch(item_id)
        formated_item_description = format_item_description(item_description_text)
        # print(formated_item_description)
        # time.sleep(10)
        sql = 'update item set description= "' +formated_item_description+ '" where item_id='+item_id
        print(sql)
        cur.execute(sql)
        con.commit()
...
if __name__ == '__main__':
    ...
    # translate_main('steam.db')import json
import sqlite3
import requests
import re
import time
...
def translate_main(db_path):
    con = sqlite3.connect(db_path)
    cur = con.cursor()
    cur.execute('select user_id,item_id,rating,review from review where review != "" and translate_review == "" ')
    columns = cur.fetchall()
    # print(columns)
    count = 0
    for column in columns:
...
if __name__ == '__main__':
    ...
    # item_description_text = item_description_fetch(730)
    # format_item_description(item_description_text)
    item_description_fetch_main('steam.db')